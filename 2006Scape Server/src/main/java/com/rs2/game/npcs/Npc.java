...
public class Npc {
	...
	public void shearSheep(Player player, int itemNeeded, int itemGiven, int animation, final int currentId, final int newId, int transformTime) {
		if (!player.getItemAssistant().playerHasItem(itemNeeded)) {
			player.getPacketSender().sendMessage("You need " + Deprecated.getItemName(itemNeeded).toLowerCase() + " to do that.");
			return;
		}
		...
		player.getPacketSender().sendMessage("You get some " + Deprecated.getItemName(itemGiven).toLowerCase() + ".");
